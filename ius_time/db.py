...
class TaskManager:
    ...
    # START FUNCTIONS
    def start_task(self, task_name: str, category: str = "Misc") -> Task | None:
        try:
            ...
            self.console.print(
                f"Starting task [info]{task.name}[/] at [info]{task.start_time.strftime(datetime_format)}[/]"
            )
            return task
        except Exception as e:
            self.console.print(
                f"[error]Could not start task [info]{task_name}[/info]! \n{e}[/error]"
            )
            return None
    ...
    def _print_end_successful(self, task: Task):
        self.console.print(
            f"[success]Task [info]{task.name}[/info] ended after [info]{TaskTime(task.total_time.total_seconds())!s}[/info] :100:"
        )

    def end_task(self, task_name: str) -> Task | None:
        try:
            with Session(self.db_engine) as session:
                task: Task | None = session.exec(
                    select(Task).where(
                        Task.name == task_name, Task.status == Status.ACTIVE
                    )
                ).first()
                if task:
                    ...
                    self.console.print(
                        f"[error]{task_name} is not an Active Task[/error]"
                    )
                    return None
        except Exception as e:
            self.console.print(
                f"[error]Could not end task [info]{task_name}[/info]: {e}[/error]"
            )
            return None

    def end_last(self) -> Task | None:
        try:
            with Session(self.db_engine) as session:
    ...
    def end_all_active(self) -> bool:
        try:
            with Session(self.db_engine) as session:
                ...
                session.commit()
            self.console.print(
                f"[success]Ended [info]{len(active_tasks)}[/info] active tasks at [info]{datetime_pst.now().strftime(datetime_format)}[/info]!"
            )
            return True
        except Exception as e:
    ...
    # TOTAL FUNCTIONS
    def sum_task_times(
        self, filter_: FilterEnum = FilterEnum.MONTH, category: str | None = None
    ) -> list[tuple[str, int]]:
        expression = select(Task.category, Task.total_time).where(
            Task.status == Status.COMPLETE
        )
        if category:
