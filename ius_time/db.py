...
class TaskManager:
    ...
    def start_task(self, task_name: str, category: str = "Misc"):
        ...
        self.console.print(
            f"Starting task [info]{task_name}[/] at [info]{start_task_time_dt.strftime(datetime_format)}[/]"
        )
        ...
        try:
            ...
            self.console.print(
                f"[error]Could not start task [info]{task_name}[/info]![/error]"
            )
    ...
    def end_task(self, task_name: str) -> bool:
        ...
        self.console.print(
            f"Ending task [info]{task_name}[/] at [info]{end_time_dt.strftime(datetime_format)}[/]"
        )

        start_time_result_row = self._connection.execute(
            "SELECT start_time FROM tasks WHERE name = ? AND status = ?",
            [task_name, self.status.ACTIVE],
        ).fetchone()
        if start_time_result_row is not None:
            start_time = start_time_result_row["start_time"]
        else:
            self.console.print(
                f"[error][info]{task_name}[/info] is not an Active Task!"
            )
            return False
        ...
        try:
            ...
            self.console.print(
                f"[success]Task [info]{task_name}[/info] ended after [info]{TaskTime(total_time)!s}[/info] :100:"
            )
            return True
        except sqlite3.Error:
            self.console.print(
                f"[error]Could not end task [info]{task_name}[/info]![/error]"
            )
            raise
    ...
    def end_all_active(self) -> bool:
        ...
        resp = self._connection.execute(
            "SELECT * FROM tasks WHERE status = ?", [self.status.ACTIVE]
        )
        ...
        if num_tasks > 0:
            try:
                with self._connection:
                    ...
                    self._connection.execute(
                        "UPDATE tasks SET end_time = ? WHERE status = ?",
                ...
                self.console.print(
                    "[error]An error occurred during attempt to end all active tasks!"
                )
                raise
        else:
            self.console.print("[error]No active tasks to end!")
    ...
    def list_complete(
        self, filter_: FilterEnum = FilterEnum.MONTH
    ) -> list[sqlite3.Row]:
        start_time = parse_filter(filter_)
        resp = self._connection.execute(
            "SELECT * FROM tasks WHERE status = ? AND start_time > ?",
    ...
    def list_all(self, filter_: FilterEnum = FilterEnum.MONTH) -> list[sqlite3.Row]:
        start_time = parse_filter(filter_)
        resp = self._connection.execute(
            "SELECT * FROM tasks WHERE start_time > ?", [start_time]
        )
        return resp.fetchall()

    # TOTAL FUNCTIONS
    def sum_task_times(
        self, filter_: FilterEnum = FilterEnum.MONTH, category: str = None
    ) -> list[sqlite3.Row]:
        start_time = parse_filter(filter_)

        if category is None:
            resp = self._connection.execute(
                "SELECT category, SUM(total_time) FROM tasks WHERE start_time > ? \
                GROUP BY category ORDER BY SUM(total_time) DESC",
                [start_time],
            )
        else:
            resp = self._connection.execute(
                "SELECT category, SUM(total_time) FROM tasks WHERE start_time > ? AND category = ? \
                GROUP BY category ORDER BY SUM(total_time) DESC",
                [start_time, category],
            )
        return resp.fetchall()
